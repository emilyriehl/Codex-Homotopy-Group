...
| 30 | Hopf fibration `S^1 -> S^3 -> S^2` and total-space equivalence | MISSING | Partial locally | `src/synthetic-homotopy-theory/hopf-fiber-sequence.lagda.md`, `src/synthetic-homotopy-theory/hopf-family-circle.lagda.md`, `src/synthetic-homotopy-theory/hopf-construction-circle.lagda.md`, `src/synthetic-homotopy-theory/hopf-construction-fiber-sequence.lagda.md`, `src/synthetic-homotopy-theory/suspensions-as-joins.lagda.md`, `src/synthetic-homotopy-theory/functoriality-joins-of-types.lagda.md`, `src/synthetic-homotopy-theory/type-arithmetic-joins-of-types.lagda.md`, `src/synthetic-homotopy-theory/spheres-as-join-powers.lagda.md`; HoTT book `thm:hopf-fibration` | The desired packaged sequence is still scaffolded, but the Hopf-family and first total-space layers are checked: `hopf-family-circle` defines the family over `S²` whose meridians are classified by left multiplication on `S¹`, packages its projection as a pointed fiber sequence with fiber `S¹`, proves by the flattening lemma that the actual family total space is a pushout of the family-induced flattened span, names the associated explicit descent data and flattened span, proves that `S¹ * S¹` is a pushout of that explicit flattened span via the Hopf shear, completes the two-leg comparison between those flattened spans, transports the total-space pushout property to the explicit span, packages `total-space-hopf-family-sphere-1 ≃ S¹ * S¹`, identifies the Hopf-family total space with the Hopf-construction total space, and composes to `total-space-hopf-family-sphere-1 ≃ join-power 2 (Fin 2) * join-power 2 (Fin 2)`; `hopf-construction-circle` and `hopf-construction-fiber-sequence` expose the analogous Hopf-construction total-space/accessor comparisons to that join-power target; `h-space-structure-circle` packages the Hopf shear equivalence needed for the span comparison; `suspensions-as-joins` proves `Fin 2 * X ≃ suspension X`; `functoriality-joins-of-types` proves functoriality and preservation of equivalences for joins; `type-arithmetic-joins-of-types` proves `A * B ≃ B * A`, right- and left-product preservation of join pushouts, the structural associator map `((A * B) * C) -> A * (B * C)`, the inverse-direction associator map `A * (B * C) -> (A * B) * C`, the right-composite homotopy for the associator followed by the right-associating auxiliary map, the inverse-side left-composite homotopy for the inverse associator followed by the left-associating auxiliary map, endpoint reductions for both the retraction and section triangles, product/left-product cocone glue computation laws, and dependent-pushout packages reducing the two-sided inverse homotopies to two normalized product/left-product triple coherence families; and `spheres-as-join-powers` proves `join-power (succ n) (Fin 2) ≃ sphere n`, including `join-power 4 (Fin 2) ≃ S^3`, plus the bridge from `S^1 * S^1` to `join-power 2 (Fin 2) * join-power 2 (Fin 2)`, the checked structural map onward to `join-power 4 (Fin 2)`, and the reduction of that map's equivalence proof to the two relevant associator equivalence instances. Next targets are proving the associator map is an equivalence, now focused on constructing those two normalized triple coherences or an equivalent universal-property proof using the product-preservation pushout infrastructure, transport to the `S³` model, and packaging of the resulting fiber sequence as `S^1 -> S^3 -> S^2`. |
...
5. **Develop the Hopf construction and Hopf fibration.** The circle and
   ...
   glue computation laws, and dependent-pushout reductions from two normalized
   product/left-product triple coherence families to the two-sided inverse
   homotopies, the bridge from
   `S^1 * S^1` to
   ...
   that map's equivalence proof to the two relevant associator equivalence
   instances. Next, prove that the structural associator map is an equivalence,
   either by constructing those two normalized triple coherences or by a
