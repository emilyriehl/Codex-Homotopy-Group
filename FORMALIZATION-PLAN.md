...
| 30 | Hopf fibration `S^1 -> S^3 -> S^2` and total-space equivalence | MISSING | Partial locally | `src/synthetic-homotopy-theory/hopf-fiber-sequence.lagda.md`, `src/synthetic-homotopy-theory/suspensions-as-joins.lagda.md`; HoTT book `thm:hopf-fibration` | The desired packaged sequence is still scaffolded, but the suspension-as-`Fin 2`-join maps and computation rules are checked; next targets are the inverse-law coherences, join associativity/join-power comparison, and final `S^1 * S^1 ≃ S^3` packaging. |
