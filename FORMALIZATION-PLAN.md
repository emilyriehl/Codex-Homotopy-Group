...
5. **Develop the Hopf construction and Hopf fibration.** The circle and
   ...
   for joins, the bridge from `S^1 * S^1` to
   ...
   `join-power 2 (Fin 2) * join-power 2 (Fin 2)`, and the checked structural
   map from that join to `join-power 4 (Fin 2)`. Next, prove that this
   associativity/join-power multiplication map is an equivalence, then use
