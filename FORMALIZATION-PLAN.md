...
| 33 | Stability of homotopy groups of spheres | MISSING | Done locally for the needed case | `src/synthetic-homotopy-theory/stability-third-homotopy-group-sphere-3.lagda.md`; HoTT book `cor:stability-spheres` | The comparison `π₂(S²) ≅ π₃(S³)` is checked from the Freudenthal/Blakers-Massey `S²` instance. |
| 34 | Diagonal theorem `pi_n(S^n) ~= Z` | MISSING | Needed `n = 3` theorem done locally | `src/synthetic-homotopy-theory/third-homotopy-group-sphere-3.lagda.md`; HoTT book `thm:pinsn` | The general diagonal theorem is not formalized, but the target route now proves `π₃(S³) ≅ ℤ` unconditionally via stability, Hopf, and `π₁(S¹) ≅ ℤ`. |
