...
| 30 | Hopf fibration `S^1 -> S^3 -> S^2` and total-space equivalence | MISSING | Partial locally | `src/synthetic-homotopy-theory/hopf-fiber-sequence.lagda.md`, `src/synthetic-homotopy-theory/suspensions-as-joins.lagda.md`; HoTT book `thm:hopf-fibration` | The desired packaged sequence is still scaffolded, but `suspensions-as-joins` now proves the upstream-style equivalence `Fin 2 * X ≃ suspension X` by comparing cocones over the `Fin 2` join span with suspension structures. Next targets are join associativity/join-power comparison and final `S^1 * S^1 ≃ S^3` packaging. |
