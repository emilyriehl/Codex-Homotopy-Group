...
| 28 | Hopf construction for connected H-spaces | MISSING | Partial locally | `src/synthetic-homotopy-theory/hopf-construction.lagda.md`; HoTT book `sec:hopf` | The generic Hopf map `A * A -> suspension A` and its pointed form are checked; the fibration proof remains. |
...
| 30 | Hopf fibration `S^1 -> S^3 -> S^2` and total-space equivalence | MISSING | Stubbed locally | `src/synthetic-homotopy-theory/hopf-fiber-sequence.lagda.md`; HoTT book `thm:hopf-fibration` | The actual maps and fiber-sequence proof remain; next target is `S^1 * S^1 ≃ S^3` plus packaging. |
