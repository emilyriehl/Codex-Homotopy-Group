...
- The first total-space comparison layers for the Hopf construction are now
  ...
  comparison map
  `join-power 2 A * join-power 2 A -> join-power 4 A`. The remaining
  total-space comparison is therefore focused on proving this associativity
  route is an equivalence, then transporting to the `S³` model.
...
1. Extend the checked Hopf map `S¹ * S¹ ->* S²` to the Hopf fiber sequence
   ...
   a checked structural map from that join to `join-power 4 (Fin 2)`. The
   remaining Hopf work is to prove that this associator/join-power
   multiplication map is an equivalence, transport the total-space comparison
   to the `S³` model, and then transport/package the result as the final
...
The next upstream-shaped target is to turn the checked pointed Hopf map `S¹ * S¹ ->* S²` into the packaged Hopf fiber sequence. The structural route now has one main total-space comparison target: prove the checked associator/join-power multiplication map `join-power 2 (Fin 2) * join-power 2 (Fin 2) -> join-power 4 (Fin 2)` is an equivalence, compose it with the checked `S¹ * S¹` and `join-power 4 (Fin 2) ≃ S³` comparisons, then transport the Hopf map across the `S³` comparison as needed and fill `src/synthetic-homotopy-theory/hopf-fiber-sequence.lagda.md` with the actual pointed maps and fiber-sequence proof.
...
comparison is now the equivalence proof for the checked structural
join-power multiplication/associativity map from
