...
- The naturality square comparing the ordinary underlying map of a concrete
  homotopy-group homomorphism with the corresponding set-truncated loop map now
  has a checked equivalence bridge: if the set-truncated loop map is an
  equivalence, then the ordinary underlying map of the induced concrete
...
- Three reusable connectivity prerequisites for the library-quality Freudenthal
  ...
  exponential characterization of connected types. The cogap of a
  pushout-product square is also proved fiberwise equivalent to the join of the
  two input fibers, and hence `(k+n+2)`-connected when the two input maps are
  `k`- and `n`-connected.
- The generalized Blakers-Massey theorem for relation-indexed span pushouts is
  now checked. The module `blakers-massey-span-pushouts` follows the
  Coq-HoTT/Rocq code-family route: it constructs the left and right code
  families, inserts the connected join hypothesis, proves the decomposition
  pushout comparison through truncation equivalences, contracts the right code
  by a transport-singleton/glue-beta computation, and packages both the
  pointwise glue-map connectedness theorem and the total gap-map connectedness
  theorem. It also includes the ordinary-span corollary for maps
  `f : S -> A` and `g : S -> B`.
...
- The group-level LES bridge has been split one level lower. The
  ...
  The later recursive/canonical boundary comparison exposed a real sign: the
  loop of the boundary agrees with the fresh shifted boundary after
  precomposition by the double-loop inversion map induced by `ap inv`. The
  checked first-loop theorem is the signed comparison
  `coherence-square-first-loop-canonical-iterated-boundary-fiber-sequence-signed`,
  not the older unsigned square. This sign is now systematic: applying the
  first-loop comparison to each iterated loop fiber sequence gives the checked
  all-index comparison
  `coherence-square-canonical-iterated-boundary-fiber-sequence-signed` and the
  exactness theorem
  `is-exact-set-truncation-loop-canonical-iterated-boundary-fiber-sequence-signed`.
  The public set-truncated LES package now instantiates the generic
  `Long-Exact-Sequence-Pointed-Set` display with one looped canonical boundary
  map. The signed comparison to the fresh shifted boundary remains in
  `signed-boundary-comparisons-fiber-sequences`, and the group-level LES package
  exposes one canonical iterated boundary homomorphism as an actual record field
  rather than as a compatibility alias layer.

- The first structural split toward a library-quality LES module layout is now
  checked. The new module `connecting-fiber-sequences` gives public
  `connect_fiberseq`-style names to the shifted connecting fiber sequence
  `Ω E ->* Ω B ->* fiber g` and its packaged-fiber-sequence form
  `Ω E ->* Ω B ->* F`. The iterated set-truncated exactness file now exposes
  connecting-sequence exactness names and keeps the older direct-boundary names
  as compatibility aliases; the group exactness file similarly exposes a
  connecting-map exactness theorem for the recursive connecting route.
- The LES module split has now extracted the boundary-adapter and base
  set-truncated exactness layers. `fibers-boundary-maps-pointed-maps` owns the
  direct fiber-of-boundary comparison and looped-boundary adapters, while
  `set-truncated-exactness-homotopy-groups-fiber-sequences` owns the base
  set-level exactness proofs for canonical, packaged, boundary, shifted
  connecting, looped packaged, and looped boundary/fiber-inclusion segments.
  The main LES file now focuses on induced homotopy-group maps, recursive
  boundary maps, and homomorphism wrappers.
- The remaining definition-level LES contents have now been extracted from
  `long-exact-sequences-homotopy-groups`. The new modules
  `iterated-loop-fiber-sequences`, `iterated-boundary-maps-fiber-sequences`,
  and `homomorphisms-homotopy-groups-fiber-sequences` own the iterated loop
  fiber sequence, recursive/canonical boundary maps, direct shifted boundary
  fiber sequence, and concrete homotopy-group homomorphism wrappers. The old
...
| Fibers of boundary maps | [`src/synthetic-homotopy-theory/fibers-boundary-maps-pointed-maps.lagda.md`](src/synthetic-homotopy-theory/fibers-boundary-maps-pointed-maps.lagda.md) | Owns the LES-facing boundary-map terminology for pointed maps, the direct equivalence between `Ω E` and the fiber of the boundary map `Ω B ->* fiber g`, the comparison with the fiber-inclusion-of-the-fiber-inclusion sequence, and the looped-boundary adapters used by iterated exactness. |
| Set-truncated LES exactness | [`src/synthetic-homotopy-theory/set-truncated-exactness-homotopy-groups-fiber-sequences.lagda.md`](src/synthetic-homotopy-theory/set-truncated-exactness-homotopy-groups-fiber-sequences.lagda.md) | Proves base set-truncated exactness for canonical and packaged fiber-sequence triples, boundary triples, shifted connecting triples, looped packaged triples, looped boundary/fiber-inclusion triples, and the initial four-triple set-truncated LES segment package. |
| Iterated loop fiber sequences | [`src/synthetic-homotopy-theory/iterated-loop-fiber-sequences.lagda.md`](src/synthetic-homotopy-theory/iterated-loop-fiber-sequences.lagda.md) | Packages the iterated loop fiber sequence `Ω^n F ->* Ω^n E ->* Ω^n B`, including the pointed equivalence from `Ω^n F` to the fiber of the iterated fibration and compatibility with the iterated fiber inclusion. |
| Iterated boundary maps of fiber sequences | [`src/synthetic-homotopy-theory/iterated-boundary-maps-fiber-sequences.lagda.md`](src/synthetic-homotopy-theory/iterated-boundary-maps-fiber-sequences.lagda.md) | Defines the recursive boundary maps, reassociation lemmas, fresh canonical iterated boundary maps, looped-canonical boundary maps, and the direct shifted boundary fiber sequence for a packaged fiber sequence. |
...
| Blakers-Massey for span pushouts | [`src/synthetic-homotopy-theory/blakers-massey-span-pushouts.lagda.md`](src/synthetic-homotopy-theory/blakers-massey-span-pushouts.lagda.md) | Proves the generalized Blakers-Massey theorem for relation-indexed span pushouts. It defines row and column total spaces, proves the ABFJ/Coq-HoTT connected-join hypothesis from row and column connectedness, formalizes the Coq-HoTT-style code-family contraction of the right code, packages connectedness of all span-pushout glue maps, derives connectedness of the total gap map, and proves the ordinary-span corollary for maps `f : S -> A` and `g : S -> B`. |
...
| Set-truncated iterated LES maps | [`src/synthetic-homotopy-theory/set-truncated-iterated-maps-homotopy-groups-fiber-sequences.lagda.md`](src/synthetic-homotopy-theory/set-truncated-iterated-maps-homotopy-groups-fiber-sequences.lagda.md) | Owns the set-truncated iterated LES maps on `Ω Ω^n F`, `Ω Ω^n E`, and `Ω Ω^n B`, including the fiber-inclusion, fibration, recursive boundary, canonical shifted boundary, looped canonical boundary, direct shifted, and connecting-sequence variants. |
| Set-truncated canonical iterated LES exactness | [`src/synthetic-homotopy-theory/set-truncated-canonical-iterated-exactness-homotopy-groups-fiber-sequences.lagda.md`](src/synthetic-homotopy-theory/set-truncated-canonical-iterated-exactness-homotopy-groups-fiber-sequences.lagda.md) | Owns the structural set-truncated exactness theorems for all-index total-space exactness, canonical shifted fibration-boundary exactness, and canonical shifted boundary/fiber-inclusion exactness. |
...
| Direct group exactness compatibility for homotopy groups | [`src/synthetic-homotopy-theory/direct-exactness-homotopy-groups-fiber-sequences.lagda.md`](src/synthetic-homotopy-theory/direct-exactness-homotopy-groups-fiber-sequences.lagda.md) | Owns the direct shifted connecting-map exactness theorem and historical direct-boundary aliases used around the Hopf LES segments. |
| Recursive group exactness compatibility for homotopy groups | [`src/synthetic-homotopy-theory/recursive-exactness-homotopy-groups-fiber-sequences.lagda.md`](src/synthetic-homotopy-theory/recursive-exactness-homotopy-groups-fiber-sequences.lagda.md) | Owns the recursive set-truncated-to-group exactness wrapper, pointed-homotopy transport, the low-dimensional boundary/fiber-inclusion statement, and the trivial-codomain fallback retained for downstream compatibility. |
| Group exactness compatibility facade for homotopy groups | [`src/synthetic-homotopy-theory/exactness-homotopy-groups-fiber-sequences.lagda.md`](src/synthetic-homotopy-theory/exactness-homotopy-groups-fiber-sequences.lagda.md) | Re-exports the canonical, direct, and recursive group exactness modules so older downstream imports keep working while proof-route ownership is explicit. |
| Group-level LES package | [`src/synthetic-homotopy-theory/long-exact-sequences-homotopy-groups-fiber-sequences.lagda.md`](src/synthetic-homotopy-theory/long-exact-sequences-homotopy-groups-fiber-sequences.lagda.md) | Defines `Long-Exact-Sequence-Homotopy-Groups-Fiber-Sequence` and instantiates it as `long-exact-sequence-homotopy-groups-fiber-sequence`, with the standard fiber-inclusion, fibration, canonical boundary, and exactness projections as fields while keeping signed transport hidden in imported exactness proofs. |
...
The full current route to `π₃(S²) ≅ ℤ` is checked and unconditional. The main completed proof chain is:

- generalized Blakers-Massey for span pushouts in `blakers-massey-span-pushouts`;
- Freudenthal for the suspension-loop unit in `freudenthal-suspension-theorem`, proved by specializing ordinary-span Blakers-Massey to `unit <- A -> unit`;
...
All eleven Agda checks passed. The old
`long-exact-sequences-homotopy-groups` file is now a thin coordination module,
and downstream set-level, group-level, and Hopf consumers import the extracted
modules directly.

On 2026-06-26, the boundary-map adapter and base set-truncated exactness
splits were checked with:
...
All eight Agda checks passed. The main LES module now imports the two new
modules instead of carrying boundary-adapter and base exactness proof blocks
inline.

On 2026-06-26, the signed loop-boundary comparison and first-loop
recursive/canonical set-truncated coherence were checked with:
...
All three checks passed. The set-level package keeps the two adjacent
canonical boundary maps separate, matching the fresh-connecting-map approach
and avoiding a public recursive/canonical boundary equality. The group-level
package now records the canonical iterated boundary homomorphism in both
boundary slots; the signed comparison stays internal to the fibration-boundary
exactness proof.

On 2026-06-25, the Freudenthal specialization and final unconditional theorem files were checked with:
...
All four Agda checks passed. Earlier MCP typechecks also reported no goals for the edited Agda modules, but `./check.sh` is the acceptance gate.
...
Later on 2026-06-25, the Freudenthal/stabilization interface was checked with:
...
Later on 2026-06-25, the pushout-product fiber/connectivity layer was checked
with:
...
Later on 2026-06-25, the dependent pushout-product and
Coq-HoTT-guided Blakers-Massey route infrastructure were checked with:
...
question-mark placeholders. The new checked layer proves the dependent
pushout-product fiber/connectivity theorem, packages ordinary pushout gap maps,
packages span-pushout total gap maps and the double-fibration replacement
equivalence, and proves that Freudenthal follows from connectedness of the
suspension pushout gap map in the `add+2-𝕋` range.
...
./check.sh src/synthetic-homotopy-theory/long-exact-sequences-homotopy-groups.lagda.md
git diff --check
rg -n "\{!!\}|allow-unsolved-metas|postulate" src/synthetic-homotopy-theory/long-exact-sequences-homotopy-groups.lagda.md
rg -n "allow-unsolved-metas|\{!!\}" src/synthetic-homotopy-theory src/group-theory src/structured-types -g "*.lagda.md"
```
...
Later on 2026-06-22, the associator inverse-homotopy route made checked
progress in `type-arithmetic-joins-of-types`. The file now proves the
right-composite homotopy

```text
map-associative-join ∘ map-right-associative-join ~ inr-join
```

and the inverse-side left-composite homotopy

```text
map-inv-associative-join ∘ map-left-associative-join ~ inl-join
```

It also proves endpoint reductions for both the retraction triangle of
`map-inv-associative-join ∘ map-associative-join` and the section triangle of
`map-associative-join ∘ map-inv-associative-join`. Direct attempts to extend
those endpoints over the inner joins exposed higher dependent-identification
coherences over the `A * B` and `B * C` glue paths; those coherences are now the
precise remaining inverse-homotopy blockers, unless the next pass instead
proves the associator equivalence by a pushout universal-property/pasting
argument.

Later on 2026-06-22, the structural join arithmetic gained checked glue
computation laws for the product-join cocone, the unswapped left-product cocone,
and the swapped `A × (B * C)` left-product cocone. The same pass also added
checked conditional packages

```text
((x : A * B) (c : C) -> triangle-family-map-inv-coherence-map-associative-join x c)
...
```text
((a : A) (y : B * C) -> triangle-family-map-associative-coherence-map-inv-associative-join a y)
...
Later on 2026-06-25, the ordinary pushout-square reduction to the
relation-indexed span-pushout gap map was checked with:
...
The span-pushout file now proves that the relation span associated to
`f : S -> A` and `g : S -> B` has a pushout equivalent to `pushout f g`, that
the induced map on the standard pullbacks of the two pushout inclusions is an
equivalence, that the canonical relation-span gap map is compatible with this
pullback comparison, and that connectedness of the relation-span gap map
transfers to connectedness of the ordinary pushout gap map. The Freudenthal file
now exposes this reduction for the suspension span `unit <- A -> unit`, so the
remaining blocker is the generalized Blakers-Massey connectedness theorem for
...
Later on 2026-06-25, the generalized Blakers-Massey theorem for span pushouts
was completed in `blakers-massey-span-pushouts`. The proof now carries the
Coq-HoTT/Rocq code-family route through the decomposition truncation
equivalence, the left-to-right code equivalence, and the final contraction of
the right code. The last computation follows the Coq-HoTT
`transport_singleton`/`code_beta_glue` pattern: the center is transported in
the total singleton, the pointwise glue beta rule is extracted using the
fixed-codomain dependent-identification equivalence, the `compute-inl` branch
comparison is cancelled, and the result is identified with the canonical
truncated fiber witness `unit-trunc (q01 , refl)`.

The file now packages:
...
- the ordinary-span corollaries
  `is-connected-map-glue-relation-map-span-pushout-Blakers-Massey` and
...
exactness file then checked the corresponding first-loop lift

```text
is-exact-hom-canonical-fibration-boundary-concrete-homotopy-group-fiber-sequence-first-loop-signed
```

as internal compatibility code. It is not part of the public LES package. This
does not yet finish the library-quality all-index canonical LES package: the
public group-facing route still needs either multiplication preservation for
the fresh shifted set-truncated boundary maps or a delooped canonical boundary
convention.
...
Later on 2026-06-26, the boundary-map API was refactored toward the canonical
public route. The long exact sequence file now names the two canonical boundary
roles separately:
...
The set-truncated canonical LES package now defines its fibration-boundary and
boundary/fiber-inclusion fields through these names. The group-level signed
first-loop theorem was moved into a private block so it remains checked but
hidden from the public LES API.
...
Later on 2026-06-26, the next structural split of the LES file was completed.
The loop-fiber equivalence was extracted to

```text
src/synthetic-homotopy-theory/loop-spaces-fibers-of-pointed-maps.lagda.md
```

This module proves the pointed equivalence `Omega (fiber g) ~=* fiber (Omega g)`
and the compatibility of the looped fiber inclusion with the canonical
inclusion of the loop-map fiber. The first fiber-inclusion fiber sequence was
also extracted to
...
This module proves the pointed equivalence identifying the fiber of
`fiber g -> E` with `Omega B` and packages `Omega B ->* fiber g ->* E` using
the standalone connecting map. The long exact sequence file now imports both
modules instead of defining those structural blocks inline. The set-truncated
iterated exactness file imports the loop-fiber module directly, avoiding an
accidental umbrella-export dependency on the large LES module.
...
All eight Agda checks passed. A touched-file scan found no holes, postulates,
unsafe termination pragmas, rewrite-rule dependency, or unsolved-meta options,
and `git diff --check` passed. At that checkpoint, the next library-quality
split target was the remaining boundary-comparison and exactness code still
embedded in `long-exact-sequences-homotopy-groups`; later sessions moved those
layers out of the main coordination module.
...
All six Agda checks passed. At that checkpoint, the next major split target
was the boundary-comparison and exactness code still embedded in
`long-exact-sequences-homotopy-groups`; later sessions moved those layers out
of the main coordination module.
...
owns the set-truncated canonical LES record and object, while
`long-exact-sequences-homotopy-groups-fiber-sequences` owns the group-level LES
record and object. The current remaining library-quality split target is the
transport-heavy exactness support layer: the all-index set-truncated
comparison/support results, the signed boundary comparison support, and the
group-level exactness wrappers below the final package modules.
...
split into reviewer-facing modules. The new module
...
comparison and exactness transport. The original iterated exactness file now
re-exports those modules and retains compatibility wrappers for the direct,
connecting, and recursive-boundary routes. The next library-quality target is
the remaining group-level exactness adapter layer below the final LES package.
...
connecting-map theorem and historical direct-boundary aliases. The new module
`recursive-exactness-homotopy-groups-fiber-sequences` owns the recursive
set-level-to-group wrapper, pointed-homotopy transport, low-dimensional
boundary/fiber-inclusion statement, and trivial-codomain fallback. The older
`exactness-homotopy-groups-fiber-sequences` module is now only a public facade
re-exporting the canonical, direct, and recursive theorem providers.
...
package imports this canonical theorem provider directly. The older
...
trivial-codomain compatibility wrappers. The next library-quality target is
splitting those compatibility wrappers by route.
...
the connecting-map route, direct aliases, and reassociation transports. The
new module
`set-truncated-recursive-iterated-exactness-homotopy-groups-fiber-sequences`
owns kernel, pointwise-homotopy, and pointed-homotopy transports from
canonical shifted boundary exactness to recursive boundary exactness. The old
`set-truncated-iterated-exactness-homotopy-groups-fiber-sequences` module is
now only a public facade re-exporting maps, canonical exactness, signed
...
- Added `abelian-long-exact-sequences-homotopy-groups-fiber-sequences`, packaging the abelian-range LES by shifting the checked concrete group-level exactness to index `succ-ℕ n`.
- Added `pointed-set-tail-long-exact-sequences-fiber-sequences`, packaging the low-degree pointed-set tail `||Omega E||_0 -> ||Omega B||_0 -> ||F||_0 -> ||E||_0 -> ||B||_0` with exactness at the three valid middle terms.
...
Request: Emily asked Codex to follow the agreed design decisions aggressively,
renaming public LES files and terms and cleaning up the display-level LES layer.
...
- Added the generic pointed-set display record
  `structured-types.long-exact-sequences-pointed-sets.Long-Exact-Sequence-Pointed-Set`.
- Renamed the public LES files to pluralized package names and updated imports.
- Removed the old compatibility alias layer from the group-level, abelian, and
  pointed-set tail LES packages; the concise names are now the record fields.
- Replaced the old set-truncated two-boundary package with an instance of the
  generic display using one looped canonical boundary convention.
- Updated the LES coordination module and current status docs to describe the new
  public surface.
