...
- The Hopf-family/descent input over `S²` is now checked. The module
  ...
  fiber of the projection over the north pole with `sphere 1`, and packages the
  projection as a pointed fiber sequence `S¹ ->* total ->* S²`.
...
1. Extend the checked Hopf map `S¹ * S¹ ->* S²` to the Hopf fiber sequence
   ...
   `S²`, Hopf-family projection fiber sequence, suspension-as-join layer,
   sphere-as-join-power layer, join
   ...
   remaining Hopf work is to compare the checked Hopf-family fiber sequence
   with the Hopf-construction total space, prove associativity/join-power
   multiplication from that join to `join-power 4 (Fin 2)`, and then transport
   the result to the final `S^1 -> S^3 -> S^2` fiber sequence.
