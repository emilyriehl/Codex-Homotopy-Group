...
- The group-level LES bridge has been split one level lower. The
  ...
  checked first-loop theorem is now the signed comparison
  `coherence-square-first-loop-canonical-iterated-boundary-fiber-sequence-signed`,
  not the older unsigned square. The approach-2 canonical route now also has a
  checked set-truncated LES package,
  ...
  overloaded boundary terminology.
...
package records the currently checked mixed-boundary exactness data in one
object.
