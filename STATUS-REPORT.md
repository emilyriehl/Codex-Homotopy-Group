...
- The first total-space comparison layers for the Hopf construction are now
  ...
  The Hopf-family total space is now checked equivalent to `S¹ * S¹`. The
  remaining total-space comparison is therefore focused on associativity and
  join-power multiplication from
  `join-power 2 (Fin 2) * join-power 2 (Fin 2)` to `join-power 4 (Fin 2)`,
  followed by transport to the `S³` model.
...
1. Extend the checked Hopf map `S¹ * S¹ ->* S²` to the Hopf fiber sequence
   ...
   comparison, and the canonical equivalence
   `total-space-hopf-family-sphere-1 ≃ S¹ * S¹` are checked. The remaining
   Hopf work is to prove associativity/join-power multiplication from that
   join to `join-power 4 (Fin 2)`, transport the total-space comparison to
   the `S³` model, and then transport/package the result as the final
...
comparison is the join-power multiplication/associativity bridge from
`join-power 2 (Fin 2) * join-power 2 (Fin 2)` to `join-power 4 (Fin 2)`.
