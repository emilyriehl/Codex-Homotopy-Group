...
Current status: partial. This bundle contains the local macOS logs available on
this machine. A second set of logs from a Linux laptop still needs to be added.
...
The local macOS import selected Codex session files whose session metadata
recorded the repository working directory and whose event timestamps overlapped
June 3 through June 27, 2026 inclusive.
...
To add the Linux laptop logs later:

1. Copy or mount the laptop's Codex session root so it is readable on this
   machine.
